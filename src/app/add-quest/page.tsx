...
const CreateQuest = () => {
  ...
  const [rewards, setRewards] = useState<{ type: string; value: number }[]>([{ type: '', value: 0 }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const newQuest = { title, description, rewards };
    try {
      await axios.post('/api/quests', newQuest); // Adjust the endpoint as needed
      // Clear form
      setTitle('');
      setDescription('');
